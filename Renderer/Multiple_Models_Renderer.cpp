...
namespace Renderer
{
    ...
    void Multiple_Models_Renderer::update(const Camera& camera, Vector3 lightPos)
    {
        for(auto& multipleModels : m_multipleModels)
        {
            ...
            for(int i = 0; i < multipleModels->m_positions.size(); i++)
            {
                ...
                if(multipleModels->m_rotations.size() > 0)
                {
                    multipleModels->model.setRotation(multipleModels->m_rotations[i]);
                }
