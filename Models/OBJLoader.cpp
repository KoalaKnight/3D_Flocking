...
void OBJLoader::setIndices()
{
    ...
    {
        int index = findSame(m_vertexPositions[i], m_vertexPositions[i + 1], m_vertexPositions[i + 2],
                             m_tempcolours[i], m_tempcolours[i + 1], m_tempcolours[i + 2],
...
int OBJLoader::findSame(float x, float y, float z, float r, float g, float b, float nx, float ny, float nz)
{
    ...
    for(int i = 0; i < m_tempVertexPositions.size(); i += 3)
    {
        if(isSame(m_tempVertexPositions[i], x) && isSame(m_tempVertexPositions[i + 1], y) && isSame(m_tempVertexPositions[i + 2], z)
           && isSame(m_colours[i], r) && isSame(m_colours[i + 1], g) && isSame(m_colours[i + 2], b)
           && isSame(m_tempNormals[i], nx) && isSame(m_tempNormals[i + 1], ny) && isSame(m_tempNormals[i + 2], nz))
        {
            return (i / 3);
        }
    }
